...
def verificar_senha_master(senha):
    db = get_db()
    cursor = db.cursor()
    cursor.execute("SELECT senha FROM usuarios WHERE perfil = 'MASTER' AND id = %s", (session['usuario_id'],))
...
def api_condominios():
    ...
    if request.method == 'GET':
        ...
        data = request.json
        cursor.execute("INSERT INTO condominios (nome, endereco, cor) VALUES (%s, %s, %s)",
        ...
        data = request.json
        cursor.execute("UPDATE condominios SET nome=%s, endereco=%s, cor=%s WHERE id=%s",
...
def api_porteiros():
    ...
    if request.method == 'GET':
        ...
        if condominio_id:
            cursor.execute(
                "SELECT id, nome, email, condominio_id, ativo FROM usuarios WHERE perfil='PORTEIRO' AND condominio_id=%s ORDER BY nome",
                (condominio_id,))
        else:
            cursor.execute(
        ...
        cursor.execute(
            "INSERT INTO usuarios (nome, email, senha, perfil, condominio_id, ativo) VALUES (%s, %s, %s, 'PORTEIRO', %s, 1)",
        ...
        if 'senha' in data and data['senha']:
            senha_hash = hash_senha(data['senha'])
            cursor.execute(
                "UPDATE usuarios SET nome=%s, email=%s, senha=%s, condominio_id=%s, ativo=%s WHERE id=%s AND perfil='PORTEIRO'",
                (data['nome'], data['email'], senha_hash, data['condominio_id'], data.get('ativo', 1), data['id']))
        else:
            cursor.execute(
                "UPDATE usuarios SET nome=%s, email=%s, condominio_id=%s, ativo=%s WHERE id=%s AND perfil='PORTEIRO'",
        ...
        cursor.execute("UPDATE usuarios SET ativo=0 WHERE id=%s AND perfil='PORTEIRO'", (porteiro_id,))
        db.commit()
        db.close()
        return jsonify({'success': True})
...
def get_condominio_dados(condominio_id):
    ...
    cursor.execute(
        "SELECT id, tipo, nome, documento, placa, telefone, casa, observacao, created_at FROM pessoas WHERE condominio_id = %s ORDER BY nome",
        (condominio_id,))
...
def exportar_excel(condominio_id):
    ...
    cursor.execute("SELECT nome FROM condominios WHERE id=%s", (condominio_id,))
    condominio = cursor.fetchone()

    cursor.execute("SELECT * FROM pessoas WHERE condominio_id = %s ORDER BY nome", (condominio_id,))
    pessoas = cursor.fetchall()
...
def porteiro_dashboard_rapido():
    ...
    db = get_db()
    cursor = db.cursor()
    cursor.execute("SELECT nome, cor, id FROM condominios WHERE id = %s", (session['condominio_id'],))
    condominio = cursor.fetchone()
    db.close()
...
def crud_pessoas():
    ...
    if request.method == 'GET':
        ...
        data = request.json
        cursor.execute("""
            UPDATE pessoas SET tipo=%s, nome=%s, documento=%s, placa=%s, telefone=%s, casa=%s, observacao=%s
        ...
        cursor.execute("DELETE FROM pessoas WHERE id=%s AND condominio_id=%s", (pessoa_id, condominio_id))
        db.commit()
        db.close()
        return jsonify({'success': True})
...
def crud_registros():
    ...
    if request.method == 'GET':
        ...
        data = request.json
        cursor.execute("""
            UPDATE registros SET tipo=%s, titulo=%s, descricao=%s, status=%s, pessoa_id=%s, cor=%s, acao=%s
            WHERE id=%s AND condominio_id=%s
        ...
        cursor.execute("DELETE FROM registros WHERE id=%s AND condominio_id=%s", (registro_id, condominio_id))
        db.commit()
        db.close()
        return jsonify({'success': True})
...
def visualizacao_consolidada(condominio_id):
    db = get_db()
    cursor = db.cursor()
    cursor.execute("SELECT nome, cor FROM condominios WHERE id = %s", (condominio_id,))
    condominio = cursor.fetchone()
    db.close()
